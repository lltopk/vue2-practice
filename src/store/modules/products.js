...
const actions = {
  //actions是异步的, 这里的shop接口是个setTimeout异步请求, 要小心, 会导致无法立马得到请求结果
  // initProducts ({ commit }) {
  ...
  // 解法1: 让store层返回 Promise，方便调用方通过await或者then等待数据加载完成
  // initProducts ({ commit }) {
  ...
  //   })
  // },
  //解法2: 让shop.api层返回promise, 然后store层使用async/await语法, 方便调用方通过await等待数据加载完成
